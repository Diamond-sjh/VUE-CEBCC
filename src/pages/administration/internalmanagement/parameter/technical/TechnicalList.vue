  ...
  <div>
      <!-- 查询条件开始 -->
        <el-row class="custquery">
          <el-form :model="form" label-width="100px" class="demo-ruleForm" size="small">
            <el-col :span="8">
              <el-form-item label="参数编码">
                <el-input
                  v-model="form.paramCode" clearable
                ></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
              <el-form-item label="参数值">
                <el-input
                  v-model="form.paramValue" clearable
                ></el-input>
              </el-form-item>
            </el-col>
            <el-col :span="8">
          ...
          <el-col :span="24">
            <el-button type="primary" plain size="small" @click="techlistquerys()">查询</el-button>
          </el-col>
        </el-row>
      <!-- 查询条件结束 -->
      <!-- 查询列表开始 -->
       <h4 style="text-align:left;">
        ...
        <el-row>
          <el-col :span="24"  class="text_right">
            <el-button type="primary" plain size="small" @click="addTechnicPara('form_add')">新增</el-button>
        ...
        <queryResTable :pageSize="20" pagePosition="right" :url="postUrl" :params="params" :pageSizes="[20]" @dataChanged="dataChangedCallback" height="300"
          ref="tableBox" :reloadData.sync="reloadData">
      ...
      <!-- 查询列表结束 -->
    <!-- 新增修改弹框 -->
    <dialogpage :title="title" :showDialog.sync="dialogVisible" :formData="form_add" :formBack="form_add1" width="60%" @open="closedialog">
      ...
      <el-row>
        <el-form :model="form_add" :rules="rules" ref="form_add" label-width="120px">
          ...
          <el-row>
            <el-col :span="11">
              <el-form-item label="参数描述" prop="paramShowmsg">
      ...
      <el-row>
        <el-col :span="24" style="text-align:center;">
          <el-button @click="dialogVisible = false" type="primary" plain size="small">关闭</el-button>
          <el-button v-show="addorchange==='add'" type="primary" plain size="small" @click="adminaddSub('form_add')">确定</el-button>
          <el-button v-show="addorchange==='change'" type="primary" plain size="small" @click="adminchangeSub('form_add')" class="but_color">确定</el-button>
        </el-col>
      </el-row>
      </template>
    </dialogpage>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        paramCode: [{required: true, validator: validatedicCode, trigger: "blur" },{ min: 1, max: 100, message: "最多输入100个字符", trigger: "blur" }],
        paramValue: [
  ...
  methods: {
    //关闭弹框
    closedialog(){
      // console.log(this.form_add.orderNo)
      //   this.form_add.orderNo='';
    },
    //获取列表选中的数据
    ...
    techlistquerys(){
      let listlegth = this.listur.length-1;
      this.techlistquery(this.listur[listlegth].dicCode);
    },
    //查询列表
    techlistquery(id) {
      let params = {
    ...
    dicCodeClick(row) {
      ...
      this.techlistquery(row.paramId);
      this.listur.push(
        {"dicname":row.paramCode,"dicCode":row.paramId},
      )
    },
    //点击根目录
    clickurl(val, index) {
    ...
    dataChangedCallback(response) {
      this.paramTableData =
        response && response ? response.paramList : [];
    },
    //新增页面
    addTechnicPara() {
    ...
    changeTechnicPara() {
      ...
      this.form_add.orderNo = Number(this.multipleSelection[0].orderNo) ; //参数说明
    },
    //新增修改弹框打开
    openDialog() { },
