    ...
    signin : function(op)
    {
        ...
        this.initOCX(function(isSucc)
        {
            ...
            else
            {
                ...
                WebAgent.Signin.trySignin(succFun, errFun, op.onInitialBeforeInvoke, op.onInitialAfterInvoke);
            }
        }, op.lang);
    },
...
if (globalWebAgent)
{
    ...
    initFun = globalWebAgent.init;
    signinFun = globalWebAgent.signin;

    // 当调用init或signin时，为了防止出错，自动把对象切换到当前WebAgent对象
    WebAgent.init = function(op)
    {
        WebAgent = curPageWebAgent;
        globalWebAgent.init = initFun;
        globalWebAgent.signin = signinFun;
        curPageWebAgent.init(op);
    };

    WebAgent.signin = function(op)
    {
        WebAgent = curPageWebAgent;
        globalWebAgent.init = initFun;
        globalWebAgent.signin = signinFun;
        curPageWebAgent.signin(op);
    };
}
else
{
